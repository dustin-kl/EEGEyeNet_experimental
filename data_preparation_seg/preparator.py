...
class Preparator:
    ...
    def run(self):
        ...
        total_samples = 0 
        chan = 0
        # find out shape of the final array 
        for subject in sorted(os.listdir(self.load_directory)):
           if os.path.isdir(self.load_directory + subject):
            ...
            for f in sorted(os.listdir(cur_dir)):
                ...
                timesamples, chan = data.shape 
                total_samples += timesamples
                chan = chan 
        """

        # initialise the array due to memory issues 
        
        #curr_time = 0
        #logging.info(f"Total samples to write: {total_samples}")
        #all_EEG = np.zeros(shape=(total_samples, chan)) 
        #all_labels = np.repeat('', total_samples)
        all_EEG = []
        all_labels = []
        ...
        for subject in progress:
            if os.path.isdir(self.load_directory + subject):
                ...
                for f in sorted(os.listdir(cur_dir)):
                    ...
                    progress.set_description('Loading ' + cur_dir + f + f' with shape {data.shape}')

                    for i, event in enumerate(events['type']):
                        
                        # extract event info 
                        event_name = event   
                        event_start_time = int(events['latency'][i])
                        event_end_time = int(events['latency'][i] + events['duration'][i])
                        event_length = event_end_time - event_start_time
                        event_eeg = np.array(data[event_start_time:event_end_time])

                        #logging.info(f"event name {event_name}")
                        #logging.info(f"event shape {event_eeg.shape}")

                        length, channels = event_eeg.shape 
                        if length == 0:
                            continue 

                        # add the data 
                        #all_EEG[curr_time:(curr_time+event_length)] = event_eeg
                        #all_labels[curr_time:(curr_time+event_length)] = np.repeat(event_name, event_length)
                        all_EEG.append(event_eeg)
                        all_labels.append(np.repeat(event_name, event_length))
                    ...
                    # comment the following if you don't want the subject names in the label 
                    #subject_label = [[subject] for i in range(len(labels))]
                    #labels = np.concatenate((labels, np.array(subject_label)), axis=1)
        ...
        logging.info(f"Failed participants: {failed_counter}")
        
        EEG = np.concatenate(all_EEG, axis=0)
        labels = np.concatenate(all_labels, axis=0)

        logging.info("Shapes of EEG are: ")
        logging.info(EEG.shape)
        logging.info("Shapes of labels are: ")
